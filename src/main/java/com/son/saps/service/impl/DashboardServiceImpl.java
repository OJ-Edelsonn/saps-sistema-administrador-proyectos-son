...
public class DashboardServiceImpl implements DashboardService {
    ...
    public DashboardServiceImpl(CursoRepository cursoRepository, DocenteRepository docenteRepository,
    ...
    public Map<String, Object> obtenerResumen() {
        ...
        resumen.put("proyectosEnDesarrollo", proyectoRepository.countByEstado(EstadoProyecto.EN_DESARROLLO));
        ...
        resumen.put("entregablesPendientes", entregableRepository.countByEstado(EstadoEntregable.PENDIENTE));
        resumen.put("observacionesCriticas", observacionRepository.countByNivelAndEstado(NivelObservacion.CRITICA, EstadoObservacion.PENDIENTE));
        BigDecimal promedio = calificacionRepository.findAll().stream()
                .map(c -> c.getNotaFinal() == null ? BigDecimal.ZERO : c.getNotaFinal())
                .reduce(BigDecimal.ZERO, BigDecimal::add);
        long totalCalificaciones = calificacionRepository.count();
        resumen.put("promedioGeneral", totalCalificaciones == 0 ? BigDecimal.ZERO : promedio.divide(BigDecimal.valueOf(totalCalificaciones), 2, RoundingMode.HALF_UP));
        resumen.put("ultimosProyectos", proyectoRepository.findTop5ByOrderByIdDesc());
        resumen.put("proximosEntregables", entregableRepository.findTop5ByEstadoOrderByFechaLimiteAsc(EstadoEntregable.PENDIENTE));
        resumen.put("observacionesPendientes", observacionRepository.findTop5ByEstadoOrderByFechaObservacionDesc(EstadoObservacion.PENDIENTE));
