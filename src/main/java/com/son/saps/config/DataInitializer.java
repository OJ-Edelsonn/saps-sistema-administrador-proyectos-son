...
public class DataInitializer {
  ...
  CommandLineRunner seedData(
    ...
    return args -> {
      Rol adminRol =
          rolRepository
              .findByNombre(RolNombre.ADMIN)
              .orElseGet(
                  () -> rolRepository.save(rol(RolNombre.ADMIN, "Administrador del sistema")));
      Rol docenteRol =
          rolRepository
              .findByNombre(RolNombre.DOCENTE)
              .orElseGet(
                  () -> rolRepository.save(rol(RolNombre.DOCENTE, "Docente asesor y evaluador")));
      Rol estudianteRol =
          rolRepository
              .findByNombre(RolNombre.ESTUDIANTE)
              .orElseGet(
                  () ->
                      rolRepository.save(
                          rol(RolNombre.ESTUDIANTE, "Estudiante integrante de grupo")));
      ...
      docentes.add(
          docente("Sofia", "Castillo Rios", "sofia.castillo@saps.edu.pe", "Calidad de software"));
      ...
      for (int i = 1; i <= 6; i++) {
        ...
        proyecto.setObjetivosEspecificos(
            "Registrar proyectos; controlar entregables; evaluar con rubricas; emitir reportes.");
      ...
      for (int i = 1; i <= 20; i++) {
        ...
        entregable.setUrlArchivo(
            "https://github.com/OJ-Edelsonn/saps-sistema-administrador-proyectos-son");
      ...
      for (int i = 1; i <= 6; i++) {
        ...
        calificacion.setComentarioGeneral(
            "Evaluacion inicial registrada para sustentar el avance del proyecto.");
